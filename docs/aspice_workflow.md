...
graph
    ...
    subgraph Phase.1 ["Phase.1: 계획 및 전략"]
        ...
        subgraph Phase.1.4
            ...
            --> MAN.3.BP5[MAN.3.BP5<br>공수 및 자원 추정]
            --> MAN.3.BP6[MAN.3.BP6<br>필요 역량 확보]
            --> MAN.3.BP7[MAN.3.BP7<br>인터페이스 관리 계획]
            --> MAN.3.BP8[MAN.3.BP8<br>프로젝트 일정 수립]
        end
    end
    ...
    subgraph Phase.3["SWE: Software Engineering"]
        ...
        subgraph SWE.1 ["SWE.1 요구사항 분석"]
            ...
            --> SWE.1.BP8[SWE.1.BP8<br>합의된 요구사항 의사소통]
        ...
        subgraph SWE.2 ["SWE.2 구조설계"]
            ...
            --> SWE.2.BP8[SWE.2.BP8<br>합의된 설계 의사소통]
        ...
        subgraph SWE.3 ["SWE.3 단위설계"]
            ...
            --> SWE.3.BP8[SWE.3.BP8<br>합의된 설계/코드 의사소통]
        end
        ...
        subgraph SWE.4 ["SWE.4 단위검증"]
            ...
            --> SWE.4.BP7[SWE.4.BP7<br>검증 결과 요약 및 의사소통]
        end
        ...
        subgraph SWE.5 ["SWE.5 통합검증"]
            ...
            --> SWE.5.BP8[SWE.5.BP8<br>테스트 결과 요약 및 의사소통]
        end
        ...
        subgraph SWE.6 ["SWE.6 적격검증"]
            ...
            --> SWE.6.BP6[SWE.6.BP6<br>테스트 결과 요약 및 의사소통]
        end
    end
    ...
    subgraph Phase.4 ["Phase.4 배포 및 프로젝트 종료"]
        ...
        subgraph Phase.4.2 ["출시 준비 및 빌드"]
            ...
            --> SPL.2.BP6[SPL.2.BP6<br>지원 유형/수준/기간 의사소통]
        end
        ...
        subgraph Phase.4.3 ["배포 및 종료"]
            ...
            --> MAN.3.BP10[MAN.3.BP10<br>프로젝트 진행률 검토 및 종료 보고]
        end
...
| MAN.3.BP5    | 프로젝트 추정치와 자원을 정의하고, 감시하고, 조정한다.                                                                                                    | 성과 2, 3, 7    |
| -            | 프로젝트의 목표, 프로젝트 위험, 동기, 경계를 기반으로 공수와 자원의 프로젝트 추정치를 정의하고, 감시하고, 조정한다.                                       |                 |
| -            | 비고4 - 적절한 추정 방법이 사용되어야 한다.                                                                                                               |                 |
| -            | 비고5 - 필요한 자원의 예로 사람, (도구, 시험 장비, 의사소통 체계 등과 같은)인프라, 하드웨어/자재가 있다                                                   |                 |
| -            | 비고6 - 프로젝트 위험(MAN.5 사용)과 품질 기준(SUP.1 사용)이 고려될 수 있다.                                                                               |                 |
| -            | 비고7 - 추정치와 자원은 일반적으로 엔지니어링, 관리, 지원 프로세스를 포함한다.                                                                            |                 |
| MAN.3.BP6    | 요구된 기술, 지식, 경험을 보장한다.                                                                                                                       | 성과 3, 7       |
| -            | 프로젝트에 요구되는 기술, 지식, 경험을 추정치와 일관되게 식별하고, 선택된 개인과 팀이 제때에 이러한 것을 보유하거나 획득하는 것을 보장한다.               |                 |
| -            | 비고8 - 요구되는 기술과 지식으로부터 편차가 있는 경우에 일반적으로 훈련이 제공된다.                                                                       |                 |
| MAN.3.BP7    | 프로젝트 인터페이스 및 합의된 책무를 식별하고, 감시하고, 조정한다.                                                                                        | 성과 4, 7       |
| -            | 다른(하위) 프로젝트, 조직 단위, 다른 영향을 받는 이해관계자와의 프로젝트 인터페이스를 식별하고 합의하고, 합의된 책무를 감시한다.                          |                 |
| -            | 비고9 - 프로젝트 인터페이스는 엔지니어링, 관리, 지원 프로세스와 관련이 있다.                                                                              |                 |
| MAN.3.BP8    | 프로젝트 일정을 정의하고, 감시하고, 조정한다.                                                                                                             | 성과 3, 5, 6    |
| -            | 활동에 자원을 할당하고 전체 프로젝트의 각 활동에 대한 일정을 수립한다. 일정은 프로젝트가 진행되는 동안 계속해서 최신화되어야 한다.                        |                 |
| -            | 비고10 - 이것은 모든 엔지니어링, 관리, 지원 프로세스와 관련이 있다.                                                                                       |                 |
| MAN.3.BP9    | 일관성을 보장한다.                                                                                                                                        | 성과 3, 4, 5, 7 |
| -            | 프로젝트에 대한 추정치, 기술, 활동, 일정, 계획, 인터페이스, 책무가 영향받는 당사자 간에 서로 일관성이 있는지를 보장한다.                                  |                 |
| MAN.3.BP10   | 프로젝트 진척 상황을 검토하고 보고한다.                                                                                                                   | 성과 6, 7       |
| -            | 영향받는 모든 당사자에게 추정된 공수와 기간 대비 활동의 이행 여부와 프로젝트의 상태를 정기적으로 검토하고 보고한다. 식별된 문제의 재발을 방지한다.        |                 |
| -            | 비고11 - 로젝트 검토는 경영진에 의해 정기적으로 수행될 수 있다. 프로젝트가 끝나는 시점에 프로젝트 검토는 모범 사례와 교훈 같은 것을 식별하는 데 공헌한다. |                 |
...
| 성과 4 | WP.08-12 / WP.13-04 / WP.14-09 / WP.14-50 / WP.15-06 |
| 성과 5 | WP.08-12 / WP.14-06 / WP.14-09                       |
| 성과 6 | WP.13-04 / WP.15-06                                  |
...
| SWE.1.BP8    | 합의된 SW 요구사항을 의사소통한다. 합의된 SW 요구사항과 갱신 내용을 관련 모든 당사자에게 의사소통한다.                         | 성과 8       |
...
| SWE.2.BP9    | 합의된 SW 아키텍처 설계를 의사소통한다. 합의된 SW 아키텍처 설계와 갱신 내용을 관련 모든 당사자에게 의사소통한다.                                       | 성과 6             |
...
| SWE.3.BP7    | 합의된 SW 상세 설계를 의사소통한다. 합의된 SW 상세 설계와 갱신 내용을 관련 모든 당사자에게 의사소통한다.                     | 성과 5          |
...
| SWE.4.BP7    | 결과를 요약하고 의사소통한다. 유닛 시험 결과와 정적 검증 결과를 요약하고 영향받는 모든 당사자에게 의사소통한다.                                                  | 성과 5 |
...
| 성과 4 | WP.13-19 / WP.13-22                       |
| 성과 5 | WP.13-04 / WP.13-25 / WP.13-50            |
...
| -            | 비고4 - 일관성은 양방향 추적성에 의해 지원되고, 검토 기록에 의해 증명될 수 있다.                                                                                 |        |
| SWE.5.BP9    | 결과를 요약하고 의사소통한다. SW 통합 시험 결과를 요약하고 영향받는 모든 당사자에게 의사소통한다.                                                                | 성과 8 |
...
| SWE.6.BP7    | 결과를 요약하고 의사소통한다. SW 인정 시험 결과를 요약하고 영향받는 모든 당사자에게 의사소통한다.                                                      | 성과 6 |
...
| SUP.1.BP4    | 품질 보증 활동과 결과를 요약하고 의사소통한다. 품질 보증 전략에 따라 품질 보증 활동의 수행, 편차, 경향을 관련 당사자에게 정기적으로 보고한다.                 | 성과 3, 4    |
| SUP.1.BP5    | 부적합 사항의 해결을 보장한다. 프로세스와 산출물 품질 보증 활동에서 발견된 편차나 부적합 사항이 분석되고, 추적되고, 수정되고, 더 나아가 예방되어야 한다.      | 성과 3, 6    |
| SUP.1.BP6    | 상신 체계를 이행한다. 품질 보증이 적절한 수준의 경영진과 이해관계자에게 문제를 상신할 수 있음을 보장하는 상신 체계를 품질 보증 전략에 따라 수립하고 유지한다. | 성과 5, 6    |
...
| 성과 3 | WP.13-04 / WP.13-07 / WP.13-18 / WP.13-19 / WP.14-02 |
| 성과 4 | WP.13-04 / WP.13-18 / WP.13-19                       |
| 성과 5 | WP.13-04 / WP.13-07 / WP.14-02                       |
| 성과 6 | WP.14-02                                             |
...
| SPL.2.BP6    | 출시에 대한 지원 유형, 지원 서비스 수준, 지원 기간을 의사소통한다. 출시에 대한 지원 유형, 지원 서비스 수준, 지원 기간이 식별되고 의사소통된다.                                         | 성과 3    |
